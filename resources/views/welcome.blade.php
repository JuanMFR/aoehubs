...
@section('title', 'AoEHubs — Plataforma ranked')

@section('content')
<div class="flex items-center justify-center min-h-[60vh]">
    <div class="w-full max-w-md text-center">
        <div class="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-xl bg-accent-dark text-accent font-bold text-2xl">
            A2
        </div>
        <h1 class="text-3xl font-bold tracking-tight">AoEHubs</h1>
        ...
        <div class="mt-8">
            @auth
                ...
                <a href="{{ route('dashboard') }}"
                   class="inline-block rounded border border-accent bg-accent-dark px-6 py-2.5 font-semibold text-accent hover:bg-accent hover:text-accent-dark transition-colors">
                    Ir al dashboard →
                </a>
            @else
                <a href="{{ route('login') }}"
        ...
        <div class="mt-12 flex flex-wrap justify-center gap-4 text-sm">
            <a href="{{ route('companion') }}" class="text-zinc-500 hover:text-zinc-300">Descargar el companion →</a>
